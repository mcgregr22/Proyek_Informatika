...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (e) {
    e.preventDefault(); // cegah langsung kirim form

    const selectedRole = roleSelect.value;
    let message = "Apakah Anda yakin ingin menyimpan perubahan role ini?";

    // ✅ Jika admin mengubah dirinya sendiri jadi pengguna
    if (selectedRole === "pengguna" && currentUserId === targetUserId) {
      message = "Anda akan mengubah <b>akun admin Anda sendiri</b> menjadi pengguna.<br><br>Jika dilanjutkan, Anda akan <b>otomatis logout</b>.";
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        form.submit(); // lanjut kirim form ke controller
      }
    });
  });
});
