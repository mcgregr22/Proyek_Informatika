...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // kelas "aktif" (warna biru) yang dipakai di sidebar
  const ACTIVE = ['bg-indigo-50','text-indigo-700','ring-1','ring-indigo-200'];

  // helper: setel item aktif
  ...
  bookSwapsLink.addEventListener('click', async (e) => {
    ...
    try {
      ...
      // aktifkan highlight di "Book Swaps" dan pastikan "Tambah Buku" tidak biru
      setActive(bookSwapsLink);

      // sembunyikan banner/strip "Library-Hub" yang mungkin ikut terbawa dari view swap
      ...
      if (strayBanner) {
        // naik sedikit agar tidak meninggalkan ruang kosong
  ...
  // kalau user klik "Tambah Buku" lagi, kembalikan highlightnya
  if (tambahBukuLink) {
